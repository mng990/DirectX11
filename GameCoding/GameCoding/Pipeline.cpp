...
void Pipeline::UpdatePipeline(PipelineInfo info)
{
	...
	// OM
	if(info.blendState) _deviceContext->OMSetBlendState(info.blendState->GetComPtr().Get(), info.blendState->GetBlendFactor(), info.blendState->GetSampleMask());	
}
...
void Pipeline::DrawIndexed(uint32 indexCount, uint32 staratIndexLocation, uint32 baseIndexLocation)
{
	_deviceContext->DrawIndexed(indexCount, staratIndexLocation, baseIndexLocation);
}
